...
`ork up` is state-aware. It loads the configured state backend before walking the component graph.
...
Then `ork up` can distinguish unchanged applied components from changed ones.
...
```bash
ork up <env-id> --reconcile
ork up <env-id> --reapply component-name
ork up <env-id> --replace component-name
```
...
```bash
ork up <env-id> --rollback-on-failure
```
