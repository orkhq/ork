...
| `ORK_WORKDIR` | Adapter operations, hooks | Component workdir on the runner. |

Lifecycle hooks also receive:

| Variable | Available in | Description |
| --- | --- | --- |
| `ORK_LIFECYCLE` | Hooks | Hook phase, such as `pre_apply` or `post_destroy`. |
...
| Adapter | Variables |
| --- | --- |
| Script | `ORK_OUTPUT_ENV`, `ORK_OUTPUT_JSON` |
